...
- **Dev mode** — task-pipeline review. Each change lives in one **开发文档** under `docs/开发文档/<module>/`. The author can **Adjust** or **Approve**; templates live in the agent skill, not the vault. Fixed bugs are distilled into the module card's **缺陷史** (symptom → root cause), which the AI reads before start/change/deliver as a regression checklist. Shared concepts (e.g. a local currency) live as concept cards under `docs/公用配置/`, referenced from docs via `[[wikilinks]]` — the card is authoritative and the AI never guesses. Legacy `开发需求/` + `开发交付/` pairs still show until migrated.
