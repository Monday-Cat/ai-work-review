...
参数：开发文档路径或任务名 / doc path or task name。填同一文档的「交付」节，不要另建交付文件 / fill the same doc, no new file。自测结果含对照模块卡「缺陷史」的回归检查 / self-test includes a regression check against 缺陷史。改动触及页面布局的，重画对应页面卡（`docs/页面文档/`）/ layout-touching changes redraw the page cards。
