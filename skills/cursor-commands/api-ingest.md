...
Normalize external API material (web page / tables / txt / word / pasted) into the per-service summary `docs/接口文档/<服务>/_接口.md`：fetch web pages with your scraping ability (login-walled pages: ask the author to save or paste), parse any local format (convert office / pdf to text first)；missing fields are filled with 暂无；endpoint names / descriptions / HTTP Method may be inferred from function names or context, confirmed by the author item by item；response JSON is always pretty-printed (```json block, one field per line)；raw snapshot saved as `_源.<ext>`, `_索引.md` refreshed.
