---
description: 记一条 BUG 进「缺陷记录」并切「整改中」（添加BUG [模块或文档]）
---

`添加 BUG` 的快捷写法：按面板同款格式把作者口述的缺陷写进指定开发文档的「缺陷记录」顶部（`### 时间戳`＋症状＋`- **整改**：` 空占位，最新在前），状态切「整改中」。只写记录不修 BUG——修用 `整改`；定位不到文档就问，不新建。面板「撤回」不覆盖 AI 记的条目，记错让 AI 删。
