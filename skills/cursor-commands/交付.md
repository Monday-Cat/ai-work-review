---
description: 对照验收填交付节，状态改已交付（交付 <路径或任务名>）
---

参数：开发文档路径或任务名。填同一文档的「交付」节，不要另建交付文件。自测结果含对照模块卡「缺陷史」的回归检查。改动触及页面布局的，重画对应页面卡（`docs/页面文档/`）。
...
执行 dev-review 的子命令「交付」，参数：$ARGUMENTS
