...
Registers the given component file into the registry：write its card at `docs/组件文档/<分类>/<组件名>.md` (path / responsibility / props API / minimal example / consumers) and refresh `_索引.md`. Use it when a new reusable component is delivered, so future development can find it via `组件查`.
